...
# Creaite Air Passangers Dataset
AirPass <- as.xts(AirPassengers)
head(AirPass)

# Create a data set called x
data <- rnorm(365*3)
dates <- dates <- seq(as.Date("2015-01-01"), length = 365*3, by = "days")
...
subset_of_dates <- as.Date("2016-01-01") + 0:4 # Adds days only
subset_of_dates
x[subset_of_dates]

# Subset x using dates as POSIXct
x[as.POSIXct(subset_of_dates)]
```
...
```{r}
dates_a <- as.Date("2016-01-01") + 0:4
dates_b <- as.Date("2016-01-01") + 0:5

a <- xts(x = 1:5, order.by = dates_a)
b <- xts(x = 1:6, order.by = dates_b)
print(cbind(a, b))
...
```{r}
dates <- as.Date("2016-01-01") + 0:4
a <- xts(x = 1:5, order.by = dates)
dates <- as.Date("2016-01-01") + 0:3
b <- xts(x = 1:4, order.by = dates)
print(cbind(a, b))

# Add a to b, and fill all missing rows of b with 0
b_filled <- merge(b, index(a), fill = 0)
print(cbind(a, b_filled))
print(a + b_filled)

# Add a to b and fill NAs with the last observation
b_filled <- merge(b, index(a), fill = na.locf)
print(cbind(a, b_filled))
print(a + b_filled)
```
...
```{r}
# URL: https://rstudio-pubs-static.s3.amazonaws.com/520452_173b0066aa024bb5b7eec5384281b726.html
library(raustats)
rba_table_cache()
rba_search(pattern = "exchange rate") #Any table relates to exchange rate#
er_tables<- rba_search(pattern = "exchange rate.+ to Current") #by adding "current" into the argument to narrow down the search
print(er_tables)
er_data<- rba_stats("F11.1") #F11.1 is the table_no.
str(er_data)
#choose the period between 2019-07-01 and 2019-07-31
er_Jul_19<- filter(er_data, date >= "2019-07-01" & date <= "2019-07-31")
  er_Jul_19[is.na(er_Jul_19)] <- 0 # replacing any *NA* into 0
...
mkt_quarterly2 <- to.quarterly(x = edhec, name = "edhec_quity", indexAt = "firstof")
```
...
edhec_years <- split(x = edhec , f = "years")
# Creates a list of 23 xts objects, one for each year in the time series and with the same variables as before

# Use lapply to calculate the cumsum for each year in edhec_years
edhec_ytd <- lapply(X = edhec_years, FUN = cumsum)

# Use do.call to rbind the results
edhec_xts <- do.call(rbind, edhec_ytd)
# Now we have both the original series and one with the
...
periodicity(temps)
# Yearly periodicity from 1912-01-01 to 1971-01-01 
...
.index(temps)
.indexwday(temps) # 0-6, Sunday is 0
.indexmon(temps)
.indexyear(temps)
